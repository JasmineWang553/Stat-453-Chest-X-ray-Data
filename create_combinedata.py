...
path_to_coronahack = '.\\Coronahack-Chest-XRay-Dataset'
df = pd.read_csv(os.path.join(path_to_coronahack,
                              'Chest_xray_Corona_Metadata.csv'), index_col=0)
...
for f in folders:
    move_files(f, path_to_radio, 'Combined_data')
...
def move_files2(arr, folder):
    ...
    for file_name in arr:
        ...
        shutil.move(os.path.join(os.path.join(path_to_coronahack, 'all_data'),
                                 file_name), os.path.join('Combined_data', folder))
